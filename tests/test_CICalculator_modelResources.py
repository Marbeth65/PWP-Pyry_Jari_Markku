...
class TestModelItem(object):
    ...
    def test_post(self, client):
        ...
        # Tests that appending to model works
        resp = client.get(self.PAYMENTPLAN_URL)
        body = json.loads(resp.data)
        assert body["model"] == "No model"
        
        valid = {"paymentplan_price":1000.0, "paymentplan_provider":"dummyprovider-12", "paymentplan_months":1}
        resp = client.post(self.RESOURCE_URL, json=valid)
...
class TestModelCollection(object):
    ...
    def test_integrityError(self, client):
        duplicate = {"manufacturer":"Toyota", "model":"Corolla", "year": 2007}
